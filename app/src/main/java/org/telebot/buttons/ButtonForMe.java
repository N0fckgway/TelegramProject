...
public class ButtonForMe extends ConnectBot implements ExecuteButton, KeyboardResponse {
    ...
    public void applyButton(Update update) {
        ...
        String message = name + ", здесь вы можете управлять каждодневными уведомлениями у себя!\n\n" +
                "Эти уведомления включены по умолчанию!\n\n" +
                "Напомню вам, что это уведомление, которое содержит сколько дней до вашего дня рождения и мотивационную фразу!";
    ...
    public SendMessage keyboardResponse(long id, String text) {
        ...
        firstRow.add(createButton("YES", "✅Включить!"));
        firstRow.add(createButton("NO", "❌Выключить!"));
        rows.add(firstRow);
